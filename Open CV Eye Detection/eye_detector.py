...
class EyeTracker:
    ...
    def processFrames(self, frame):
        ...
        faces = self.face_cascade.detectMultiScale(grayscale, scaleFactor=1.1, minNeighbors=2, flags=0 | cv2.CASCADE_SCALE_IMAGE, minSize=(150, 150))
        if len(faces) == 0:
        ...
        cv2.putText(frame, 'Face', (x + w, y + h), font, 0.5, (0, 0, 139), 2, cv2.LINE_AA)
        cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 0, 139), 2)
        # color_face = frame[y:y + h, x:x + w]
        grayscale_face = grayscale[y:y + h, x:x + w]
        eyes = self.eye_cascade.detectMultiScale(grayscale_face, scaleFactor=1.1, minNeighbors=2, flags=0 | cv2.CASCADE_SCALE_IMAGE, minSize=(30, 30))
        if len(eyes) != 2:
        ...
        for eye in eyes:
            ex, ey, ew, eh = eye
            cv2.rectangle(frame, (x + ex, y + ey), (x + ex + ew, y + ey + eh), (0, 140, 255) if eye[0] == left_eye[0] else (0, 0, 139), 2)
        ...
        circles = cv2.HoughCircles(left_eye, cv2.HOUGH_GRADIENT, 1, left_eye.shape[1]//8, 250, 15, left_eye.shape[0]/8, left_eye.shape[0]//3)
        if len(circles) > 0:
            print(circles)
            iris = self.getIris(left_eye, circles)
